...
# Claude-over-ACP session modes, extracted to a fork-only module so this
# heavily-churned upstream file carries call sites only.
# _acp_permission_mode_info / _acp_system_prompt_mode_info look unused here
# and are NOT: methods_config.py's handler bodies are rebound onto this
# module's globals at install time (method_ctx.py) and resolve them from
# here at call time. Dropping either import breaks config.get.
from tui_gateway.acp_session_modes import (  # noqa: F401
    _acp_permission_mode_info,
    _acp_system_prompt_mode_info,
    acp_session_info_blocks,
    apply_session_acp_modes,
    handle_acp_config_set,
)
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    info: dict = {
        ...
        # Claude-over-ACP mode blocks (acp_permission, acp_system_prompt_mode).
        **acp_session_info_blocks(
            session,
...
def _(rid, params: dict) -> dict:
    ...
    # Claude-over-ACP session modes. Fork-only; returns None when the key is
    # neither permission_mode nor acp_system_prompt_mode, letting this chain
    # carry on. The four helpers are injected so that module never imports
    # this one -- the reverse import would be a cycle.
    _acp_result = handle_acp_config_set(
        key,
        value,
        session,
        rid,
